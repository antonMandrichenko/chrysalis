...
export const getCurrentKeyCode = e => {
  const getMapData = keyMapping.get(e.keyCode);
  if (typeof getMapData === "number") {
    return getMapData;
  }
  if (getMapData.left) {
    return e.location === 1 ? getMapData.left : getMapData.right;
  }
  if (getMapData.withShift) {
    return e.shiftKey ? getMapData.withShift : getMapData.current;
  }
};
